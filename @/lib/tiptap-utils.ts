...
export const MAX_DOCUMENT_SIZE = 10 * 1024 * 1024 // 10MB per documenti
...
export const handleImageUpload = async (
  ...
  // ============================================
  // OPZIONE 1: Usa URL locale (per testing - DEFAULT)
  // Crea un blob URL locale, utile per testing senza server
  // ============================================
  ...
  // ============================================
  // OPZIONE 2: Upload a un server REST API
  // Decommenta questa sezione e commenta l'opzione 1 per usare un server
  // ============================================
  ...
  //         // Assumi che il server ritorni { url: "..." } o direttamente l'URL come stringa
  //         const imageUrl = response.url || response.data?.url || xhr.responseText
  //         resolve(imageUrl)
  //       } catch {
  //         // Se la risposta non è JSON, usa direttamente la risposta
  ...
  // ============================================
  // OPZIONE 4: Simulazione (per testing)
  // ============================================
...
/**
 * Handles file upload (PDF, documents, etc.) with progress tracking and abort capability
 * @param file The file to upload
 * @param onProgress Optional callback for tracking upload progress
 * @param abortSignal Optional AbortSignal for cancelling the upload
...
export const handleFileUpload = async (
  ...
  if (file.size > MAX_DOCUMENT_SIZE) {
    throw new Error(
      `File size exceeds maximum allowed (${MAX_DOCUMENT_SIZE / (1024 * 1024)}MB)`
    )
  }

  // Usa la stessa logica di handleImageUpload
  // Per file generici, possiamo usare blob URL o upload a server
  return new Promise((resolve, reject) => {
    if (abortSignal?.aborted) {
      reject(new Error("Upload cancelled"))
      return
    ...
    // Per file generici, creiamo un blob URL locale
    // In produzione, sostituisci con upload a server
    onProgress?.({ progress: 50 })
    
    // Crea blob URL direttamente dal file
