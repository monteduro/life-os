...
export default function LoginForm({ onSwitchToRegister, onForgotPassword, passwordResetSuccess }: Props) {
  ...
  // Email to verify: set when the backend responds with 403 email_unverified
  const [unverifiedEmail, setUnverifiedEmail] = useState<string | null>(null)
  // Success banner shown after a successful email verification
  ...
  function handleSubmit(e: FormEvent) {
    ...
    login(
      ...
      {
        onError: (err) => {
          // If the email is not verified, show the verification form
  ...
  // Show the email verification form when login returns email_unverified
  if (unverifiedEmail) {
    return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-md p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Error (excluding EmailUnverifiedError, already handled above) */}
